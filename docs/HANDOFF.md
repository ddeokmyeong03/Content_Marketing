...
2. **타깃 발굴 실 API 검증** — 구현은 끝났으나 실제 해시태그 검색·business_discovery를
   호출해 보지 못했다. 특히 **주간 30개 한도는 소진하면 일주일을 기다려야 하므로**
   처음에는 해시태그를 1~2개로 시작할 것
3. **SaaS 전환 준비** — 멀티테넌트(계정별 DB/설정), 인증, 잡 큐 교체

### 알아둘 것 (이번 작업에서 확인)
- **Threads 퍼블리셔는 여전히 고정 5초 대기**를 쓴다. IG처럼 폴링으로 바꾸는 것이
  맞지만, 실 API로 응답 필드(`status`)를 확인할 수 없어 보류했다. 라이브 검증 때
  같이 확인할 것
- `Platform.BOTH` 중복 발행은 **해결됐다** — `post_publications` 테이블에 플랫폼별
  발행 성공을 즉시 기록하고, 재시도는 아직 안 올라간 플랫폼만 집어간다. 부분 성공은
  알림 본문에 "이미 발행됨: instagram"으로 드러난다. 구 DB는 `init_db` 재실행
  (`python scripts/setup_db.py`)으로 테이블이 생긴다
...
- **키 교체는 `branding secrets rotate`** — 옛 키로 읽어 새 키로 다시 쓴다. 한
  트랜잭션에 커밋 전 검증까지 하므로 실패하면 아무것도 안 바뀐다(절반만 바뀌면 어느
  키로도 못 읽는다). DB를 먼저 바꾸고 그 다음에 키 파일을 갱신한다. **키를 환경변수로
  관리하면 새 키가 화면에 출력되며, 환경변수를 직접 바꿔야 한다** — 바꾸기 전까지
  읽히지 않는다. **DB 백업본은 여전히 옛 키로 암호화돼 있으니 옛 키를 함께 보관하거나
  백업도 같이 폐기할 것**
