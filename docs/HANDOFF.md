...
**테스트 189건, 전부 오프라인(네트워크 모킹).** `pytest -q`
(PNG 굽기 2건만 브라우저 필요 — 없으면 자동 skip되어 187 통과)
...
2. **DFY 고객 수용 전 보안** — 토큰·API 키 평문 저장(sqlite) 암호화, 대시보드 인증
   (현재 무인증 · 기본 바인딩 127.0.0.1). 남의 계정 자격증명·저장소 키를 받는 순간 필수
