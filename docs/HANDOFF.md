...
- [ ] **업로드(1c)** — 렌더한 PNG를 올려 `slide.image_url` 채우기. **이것만 되면 IG 발행이 열림**
- [ ] **검토 큐에 슬라이드 표시(1d)** — 대시보드가 `caption_ko`+해시태그만 보여줌.
      캐러셀을 승인하려면 슬라이드 내용을 봐야 함
...
**테스트 160건, 전부 오프라인(네트워크 모킹).** `pytest -q`
(PNG 굽기 2건만 브라우저 필요 — 없으면 자동 skip되어 158 통과)
...
### 🟠 기능 공백
4. **이미지 업로드 미구현** — Instagram 발행에 남은 **유일한** 블로커.
   문구 생성·카드 렌더링·발행 경로가 모두 연결됐고, `slide.image_url`만 비어 있음
   - 렌더한 PNG(`slide.image_path`)를 공개 URL로 올리는 단계만 남음
   - 실험 결과: Higgsfield(nano_banana)로 생성 시 **공개 CDN URL이 자동으로 나옴**(호스팅 해결됨).
     텍스트 카드는 이미 로컬 PNG로 나오므로, 업로드 경로만 정하면 됨
...
2. **업로드(1c) + 검토 큐 슬라이드 표시(1d)** — 문구 생성·렌더링까지 끝났으므로
   **업로드만 붙이면 IG 발행이 열린다.** 1d 없이는 사람이 캐러셀을 검토할 수 없어 같은 묶음
